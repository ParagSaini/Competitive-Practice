...
// same as the dijkstra, we have to maintain a visited array and only storing the curWeight 
vector<int> primAlgo(int src, unordered_map<int, vector<pair<int, int>>> graph )
{
    ...
    while(!setds.empty()) {
        ...
        for(auto it : graph[minWeightNode.second]) {
            ...
            if(adjDist < dist[adjNode] && visited[adjNode] == false) {
                if(dist[adjNode] != INT_MAX) {
                    setds.erase({dist[adjNode], adjNode});
                }
                dist[adjNode] = adjDist;
                setds.insert(make_pair(adjDist, adjNode));
            }
        }

        visited[minWeightNode.second] = true;
...
int main() {
    ...
    graph[4] = {{6,1}, {3,3}, {1,5}};
    graph[5] = {{1,4}, {11,3}};
    vector<int> ans = primAlgo(0, graph);
